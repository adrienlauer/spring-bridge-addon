...
class SpringModule extends AbstractModule {
    private static class BeanDefinition {
        ...
        BeanDefinition(String name, ConfigurableListableBeanFactory beanFactory) {
            this.name = name;
    ...
    private final ConfigurableListableBeanFactory beanFactory;
    private final Map<Class<?>, Map<String, BeanDefinition>> beanDefinitions;
    ...
    private void bindTransactionHandlers() {
        String[] beanNamesForTransactionManager = BeanFactoryUtils.beanNamesForTypeIncludingAncestors(beanFactory,
                AbstractPlatformTransactionManager.class);
        if (beanNamesForTransactionManager.length > 0) {
            for (String beanNameForTransactionManager : beanNamesForTransactionManager) {
                ...
                SpringTransactionHandler transactionHandler = new SpringTransactionHandler(transactionStatusLink,
                        beanNameForTransactionManager);
                requestInjection(transactionHandler);
                bind(Key.get(SpringTransactionHandler.class, Names.named(beanNameForTransactionManager))).toInstance(
                        transactionHandler);
            }
        }
    }
    ...
    private void bindFromApplicationContext() {
        ...
        do {
            for (String beanName : currentBeanFactory.getBeanDefinitionNames()) {
                org.springframework.beans.factory.config.BeanDefinition beanDefinition =
                        currentBeanFactory.getMergedBeanDefinition(
                        beanName);
                if (!beanDefinition.isAbstract()) {
                    ...
                    if (FactoryBean.class.isAssignableFrom(beanClass)) {
                        ...
                        if (beanClass == null) {
                            LOGGER.warn("Cannot bind spring bean " + beanName + " because its factory bean cannot " +
                                    "determine its class");
                            continue;
                        }
                    }

                    BeanDefinition springBeanDefinition = new BeanDefinition(beanName, currentBeanFactory);
            ...
            if (factory != null) {
                if (factory instanceof ConfigurableListableBeanFactory) {
                    ...
                    LOGGER.info(
                            "Cannot go up further in the bean factory hierarchy, parent bean factory doesn't " +
        ...
        for (Map.Entry<Class<?>, Map<String, BeanDefinition>> entry : this.beanDefinitions.entrySet()) {
            Class<?> type = entry.getKey();
            Map<String, BeanDefinition> definitions = entry.getValue();

            // Bind by name for each bean of this type and by type if there is no ambiguity
            for (BeanDefinition candidate : definitions.values()) {
                if (debugEnabled) {
                ...
                bind(type).annotatedWith(Names.named(candidate.getName()))
                        .toProvider(new BeanProvider(type, candidate.getName(), candidate.getBeanFactory()));
            }
        }
    }

    private void addBeanDefinition(Class<?> beanClass, BeanDefinition beanDefinition) {
        Map<String, BeanDefinition> beansOfType = this.beanDefinitions.computeIfAbsent(beanClass, k -> new HashMap<>());

        // If there are overriding beans, the first encountered bean wins (the lowest in the context hierarchy)
        if (!beansOfType.containsKey(beanDefinition.getName())) {
            beansOfType.put(beanDefinition.getName(), beanDefinition);
        }
    }
}
